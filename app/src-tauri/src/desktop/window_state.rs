        ...
        Ok(path) => {
            if path.exists() {
                match fs::read_to_string(&path) {
                    Ok(content) => match serde_json::from_str::<AppWindowState>(&content) {
                        Ok(state) => {
                            return state;
                        }
                        Err(e) => {
...
pub fn save_window_state(app: &AppHandle, state: &AppWindowState) {
    match get_window_state_path(app) {
        Ok(path) => match serde_json::to_string_pretty(state) {
            Ok(content) => {
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        if let (Ok(size), Ok(is_maximized), Ok(is_minimized)) = (
            ...
            if is_minimized || width < MIN_WINDOW_WIDTH || height < MIN_WINDOW_HEIGHT {
                println!(
                    "Skipping window state save - minimized: {}, size: {}x{} (min: {}x{})",
            ...
            println!(
                "Saved main window state: {}x{}, maximized: {}",
